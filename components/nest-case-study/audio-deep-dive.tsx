...
async function loadTranscript() {
  ...
  return { paragraphs, wordCount, readMinutes };
}

export async function AudioDeepDive() {
  const { paragraphs, wordCount, readMinutes } = await loadTranscript();
    ...
    <SectionFrame id="listen" number="11" eyebrow="Deep dive">
      ...
      <div className="mt-12 rounded-2xl border border-white/[0.08] bg-white/[0.01] p-6 md:p-10">
        ...
        <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.18em] text-subtle">
          Produced April 2026 &middot; 20:01 listen &middot; {readMinutes} min read &middot; {wordCount.toLocaleString()} words
        </p>

        <details className="group mt-8 border-t border-white/[0.06] pt-6">
          <summary className="flex cursor-pointer items-center justify-between gap-6 font-mono text-[11px] uppercase tracking-[0.18em] text-muted transition-colors hover:text-foreground">
            <span>
              Read the transcript
              <span className="ml-2 normal-case tracking-normal text-subtle">
                &middot; {readMinutes} min
              </span>
            </span>
            <span className="text-accent transition-transform group-open:rotate-45">+</span>
          </summary>

          <div className="mt-8 max-w-[65ch] space-y-6 text-base leading-[1.75] text-foreground/80 md:text-[17px]">
            {paragraphs.map((paragraph, index) => (
              <p
                key={index}
                className={
                  index === 0
                    ? "first-letter:float-left first-letter:mr-2 first-letter:font-serif first-letter:text-5xl first-letter:leading-[0.9] first-letter:text-foreground md:first-letter:text-6xl"
                    : undefined
                }
              >
                {paragraph}
            ...
            <p className="mt-10 border-t border-white/[0.06] pt-6 font-mono text-[10px] uppercase tracking-[0.18em] text-subtle">
              End of transcript &middot; generated by NotebookLM from this case study,
              then hand-edited &middot; April 2026.
            </p>
          </div>
        </details>
      </div>
